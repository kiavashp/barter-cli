...
function proxyMethod(To, From, Method) {
    To[Method] = From[Method].bind(From);
}
function readLineInstanceProxy(method) {
    CLI.prototype[method] = function () {
        var _this = this;
        return _this.readline[method].apply(_this.readline, arguments);
    };
}
...
CLI.prototype.commands = function commands(commands) {
...
CLI.prototype.onMissingCommand = function onMissingCommand(handler) {
    var _this = this;
...
readLineInstanceProxy('prompt');
readLineInstanceProxy('on');
readLineInstanceProxy('close');
readLineInstanceProxy('pause');
readLineInstanceProxy('resume');
readLineInstanceProxy('write');
readLineInstanceProxy('clearLine');

proxyMethod(CLI, readline, 'clearScreenDown');
proxyMethod(CLI, readline, 'createInterface');
proxyMethod(CLI, readline, 'cursorTo');
proxyMethod(CLI, readline, 'moveCursor');
